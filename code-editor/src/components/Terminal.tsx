      ...
      case "npm": {
        ...
        if (subCmd === "install" || subCmd === "i" || subCmd === "add") {
          ...
          try {
            ...
            addLine("warn", `⚠️  Servidor offline (${apiErr.message}) — modo SIMULADO ativado`);
            addLine("warn", `    Vou adicionar ao package.json mas NÃO baixar o código de verdade.`);
            addLine("warn", `    Pra instalar de verdade: troque pra modo "⚡ Real" (botão no terminal).`);
            for (const pkgName of pkgNames) {
              const info = PACKAGES_DB[pkgName.toLowerCase()];
              const version = info?.version || "latest";
              ...
              addLine("success", `  ✓ ${pkgName}@${version} ESCRITO em package.json (sem código real)`);
              if (info?.description) addLine("info", `    ${info.description}`);
            }
            addLine("warn", pkgNames.length > 0
              ? `⚠️  ${pkgNames.length} pacote(s) só no package.json. Sem node_modules.`
              : "✓ package.json processado");
            addLine("info", `💡 Abra o package.json pra ver — a entrada está lá.`);
          }
